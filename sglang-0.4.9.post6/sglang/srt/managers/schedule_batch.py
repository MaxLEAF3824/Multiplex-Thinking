...
class Req:
    ...
    def __init__(
        ...
        used_topk: Optional[int] = None,
        enable_entropy_mask: Optional[bool] = False,
        entropy_mask_threshold: Optional[float] = None,
        think_end_str: Optional[str] = None,
        think_end_token_id: Optional[int] = None,
        early_stopping_entropy_threshold: Optional[float] = None,
        early_stopping_length_threshold: Optional[int] = None,
        dirichlet_alpha: Optional[float] = None,
        enable_gumbel: Optional[bool] = None,
        enable_max_topk: Optional[bool] = None,
        ...
        after_thinking_temperature: Optional[float] = None,
        after_thinking_top_p: Optional[float] = None,
        after_thinking_top_k: Optional[int] = None,
        after_thinking_min_p: Optional[float] = None,
        # ==========
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    max_topk: Optional[int] = None
    used_topk: Optional[int] = None
    enable_entropy_mask: Optional[bool] = None
    entropy_mask_threshold: Optional[float] = None
    early_stopping_entropy_threshold: Optional[float] = None
    early_stopping_length_threshold: Optional[int] = None
    dirichlet_alpha: Optional[float] = None
    ...
    def init_new(
        ...
        return cls(
            ...
            used_topk=model_config.used_topk,
            enable_entropy_mask=model_config.enable_entropy_mask,
            entropy_mask_threshold=model_config.entropy_mask_threshold,
            early_stopping_entropy_threshold=model_config.early_stopping_entropy_threshold,
            early_stopping_length_threshold=model_config.early_stopping_length_threshold,
            dirichlet_alpha=model_config.dirichlet_alpha,
            enable_gumbel=model_config.enable_gumbel,
            enable_max_topk=model_config.enable_max_topk,
            gumbel_tau=model_config.gumbel_tau,
            enable_replacement=model_config.enable_replacement,
            enable_gumbel_after_thinking=model_config.enable_gumbel_after_thinking,
            enable_unweighting=model_config.enable_unweighting,
            after_thinking_temperature=model_config.after_thinking_temperature,
            after_thinking_top_p=model_config.after_thinking_top_p,
            after_thinking_top_k=model_config.after_thinking_top_k,
            after_thinking_min_p=model_config.after_thinking_min_p,
            # ==========
            # end of soft thinking
            # ==========
        )
    ...
    def get_model_worker_batch(
        ...
        if self.enable_soft_thinking:
            if self.forward_mode.is_decode():
                soft_thinking_reqs = [req for req in self.reqs if hasattr(req, 'topk_prob') and req.topk_prob is not None]
                if soft_thinking_reqs:
                    batch_topk_probs = []
                    batch_topk_indices = []
                    for req in self.reqs:
                        if hasattr(req, 'topk_prob') and req.topk_prob is not None:
                            batch_topk_probs.append(req.topk_prob)
                            batch_topk_indices.append(req.topk_idx)
                        else:
                            if self.max_topk is not None:
                                default_topk_prob = torch.zeros(self.max_topk, dtype=torch.float, device=self.device)
                                default_topk_idx = torch.zeros(self.max_topk, dtype=torch.long, device=self.device)
                                default_topk_prob[0] = 1.0
                                if req.output_ids:
                                    default_topk_idx[0] = req.output_ids[-1]
                                else:
                                    default_topk_idx[0] = 0
                                batch_topk_probs.append(default_topk_prob)
                                batch_topk_indices.append(default_topk_idx)
                            else:
                                continue
                    
                    topk_probs = torch.stack(batch_topk_probs)
                    topk_indices = torch.stack(batch_topk_indices)
